...
func googleAuth(ctx *jsonrpc.ReqCtx, db DB, certMgr *CertMgr) error {
	...
	if !ok {
		...
		if err != nil {
			return fmt.Errorf("Failed to generate client certificate for user %v: %v", user.ID, err)
		}

		// now save the full user info complete with the new cert & key
		if err := db.SaveUser(user); err != nil {
			return fmt.Errorf("Failed to persist user information along with the client-certificate: %v", err)
		}
	}
